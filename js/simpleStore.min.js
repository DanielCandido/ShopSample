...
var simpleStore = {
    ...
    extend: function(a, b, c) {
        var d;
        "undefined" == typeof b && (b = a, a = simpleStore);
    ...
    render: function(a, b) {
        var c = a.split("/")[0],
            d = {
                "": function() {
                    simpleStore.renderProducts(simpleStore.products, b)
                },
                "#product": function() {
                    var c = a.split("#product/")[1].trim();
                    simpleStore.renderSingleProduct(c, b)
                },
                "#cart": function() {
                    simpleStore.renderCart(b)
                }
            };
        d[c] ? d[c]() : simpleStore.renderError(b)
    },
        ...
        b.cartContainer.hide(), b.container.fadeOut(b.fadeSpeed, function() {
            ...
            a.forEach(function(a, e) {
                var f = $("#products-template").html(),
                    g = $(f);
                g.first().addClass(c), simpleStore.insertData(g, a);
                var h = g.find(".simpleStore_getDetail");
    ...
    renderProductOptions: function(a, b) {
        ...
        return a.forEach(function(b) {
            if (simpleStore.productPageOptions in b) simpleStore.renderProductPageOptions(b);
            else {
                var d = "",
    ...
    renderSingleProduct: function(a, b) {
        b.container.fadeOut(b.fadeSpeed, function() {
            var c = $("#product-detail-template").html(),
                d = $(c);
            simpleStore.products.forEach(function(c) {
                if (c.id == a) {
                    if (simpleStore.insertData(d, c), b.container.html(d), c.options.length) {
                        var e = simpleStore.renderProductOptions(c.options, b);
                        $(".simpleStore_options").append(e)
                    }
    ...
    initJSON: function(a) {
        ...
        $.get(a.JSONFile).success(function() {
            $.getJSON(a.JSONFile, function(a) {
                simpleStore.setProducts(a.products)
            }).fail(function() {
                simpleStore.handleFailure(a, b)
            })
        }).fail(function() {
            simpleStore.handleFailure(a, b)
        })
    ...
    validatePrices: function(a) {
        ...
        if ("JSON" === a.mode) $.get(a.JSONFile).success(function() {
            $.getJSON(a.JSONFile, function(d) {
                var e = d.products;
                simpleStore.verifyCheckoutData(b, e, !0) ? simpleStore.checkout(a, b) : simpleStore.renderError(a, c)
            }).fail(function() {
                simpleStore.handleFailure(a, c)
            })
        }).fail(function() {
            simpleStore.handleFailure(a, c)
        });
            ...
            simpleStore.plugins[d] && simpleStore.plugins[d].validate(b)
        }
    },
    setProducts: function(a, b) {
        a.length > 0 && a.forEach(function(a, b) {
            a.id = b + 1, simpleStore.products.push(a)
    ...
    generateStore: function() {
        ...
        switch (this.checkMode(a), $(window).on("hashchange", function() {
            simpleStore.render(window.location.hash, a)
        }), a.mode) {
            case "JSON":
                this.initJSON(a);
                break;
            case "Google":
                if (simpleStore.plugins.google) simpleStore.plugins.google.init(function(b) {
                    simpleStore.setProducts(b, a)
                });
                else {
        ...
        this.generateCart(a), this.setLayout(a), $(".simpleStore_checkout").on("click", function(b) {
            b.preventDefault(), simpleStore.validatePrices(a)
        }), $(".simpleStore_viewCart").on("click", function(a) {
